...
const MatrixDisplay = ({ title, matrix, subtitle, isDark }) => (
  <div className={`border rounded-2xl p-4 space-y-2 ${
    isDark ? 'bg-[#272822] border-[#3e3d32]' : 'bg-slate-100 border-slate-200'
  }`}>
    <div className={`flex justify-between items-center pb-2 border-b ${
      isDark ? 'border-[#3e3d32]' : 'border-slate-200'
    }`}>
      <h4 className={`font-medium text-xs ${isDark ? 'text-[#f8f8f2]' : 'text-slate-700'}`}>{title}</h4>
      {subtitle && <span className={`text-[10px] font-mono font-normal ${isDark ? 'text-[#75715e]' : 'text-slate-500'}`}>{subtitle}</span>}
    </div>
    <pre className={`font-mono font-normal text-xs overflow-x-auto p-2 leading-relaxed ${
      isDark ? 'text-[#a6e22e]' : 'text-indigo-600'
    }`}>
      {matrix.map((row, i) => (
...
const StatsDisplay = ({ title, stats, isDark }) => (
  <div className={`border rounded-2xl p-4 space-y-3 ${
    ...
    <h4 className={`font-medium text-[11px] uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
      Estadísticas ({title})
    </h4>
    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
      <div className={`border p-2 rounded-xl text-center ${
        ...
        <span className="font-mono text-xs font-normal text-indigo-400">{stats.max.toFixed(2)}</span>
      </div>
      ...
      <div className={`border p-2 rounded-xl text-center ${
        ...
        <span className="font-mono text-xs font-normal text-purple-400">{stats.min.toFixed(2)}</span>
      </div>
      ...
      <div className={`border p-2 rounded-xl text-center ${
        ...
        <span className="font-mono text-xs font-normal text-sky-400">{stats.avg.toFixed(2)}</span>
      </div>
      ...
      <div className={`border p-2 rounded-xl text-center ${
        ...
        <span className="font-mono text-xs font-normal text-emerald-400">{stats.sum.toFixed(2)}</span>
      </div>
